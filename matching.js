...
var MatchingMixin = {
    ...
    data: function () {
        return {
             ...
             */
            editingRule: {
                id: null, name: '', pool_id: null, section: 'NRE', pair_type: 'LS',
    ...
    methods: {
        ...
        openRuleModal: function () {
            ...
            var showRuleModal = function () {
                ...
                if (ruleEl) {
                    ...
                    // Инициализируем Select2 ностро-банка после открытия модалки
                    // (нужен готовый DOM и правильный dropdownParent).
                    ruleEl.addEventListener('shown.bs.modal', function () {
                        self._initRulePoolSelect2();
                    }, { once: true });
                }
        ...
        /**
         * Инициализирует Select2 выбора ностро-банка в форме правила.
         *
         * Пустой выбор (clear/placeholder) означает общее правило
         * (`editingRule.pool_id = null`), выбор банка ограничивает правило им.
         * Пересоздаётся при каждом открытии модалки, чтобы подхватить актуальный
         * список `accountPools` и текущее значение `editingRule.pool_id`
         * (в т.ч. авто-подставленный банк при создании из сайдбара).
         *
         * @returns {void}
         */
        _initRulePoolSelect2: function () {
            var self = this;
            if (typeof $ === 'undefined' || !$.fn || !$.fn.select2) return;
            var $el = $('#rule-pool-select2');
            if (!$el.length) return;
            ...
            $el.select2({
                ...
                theme:              'bootstrap-5',
                placeholder:        'Общее правило — все ностро-банки',
                allowClear:         true,
            ...
            $el.on('select2:select', function (e) {
                self.editingRule.pool_id = parseInt(e.params.data.id, 10);
            });
            $el.on('select2:clear', function () {
                self.editingRule.pool_id = null;
            });

            // Отображаем текущее значение (правило банка / авто-подставленный банк).
            if (self.editingRule.pool_id) {
                $el.val(String(self.editingRule.pool_id)).trigger('change.select2');
            } else {
        ...
        showAddRuleModal: function () {
            // Автоподстановка ностро-банка, если пользователь уже находится в
            // одном банке: сайдбар выверки (selectedPool) или единственный банк
            // в фильтре на /all-nostro.
            var autoPoolId = null;
            if (this.selectedPool) {
                autoPoolId = this.selectedPool.id;
            } else if (this.filters && Array.isArray(this.filters.pool_ids) && this.filters.pool_ids.length === 1) {
                autoPoolId = parseInt(this.filters.pool_ids[0], 10);
            }

            this.editingRule = {
                id: null, name: '', pool_id: autoPoolId, section: 'NRE', pair_type: 'LS',
                match_dc: true, match_amount: true, match_value_date: true,
                match_instruction_id: false, match_end_to_end_id: false,
                match_transaction_id: false, match_message_id: false,
                cross_id_search: false, is_active: true, priority: 100, description: ''
...
class MatchingService
{
    ...
    private function advanceJob(array &$state, string $jobId): void
    {
        ...
        if ($state['rule_pools'] === null) {
            $state['rule_pools']      = $this->resolvePoolIds($companyId, $accountId, $scopeIds, $rule->pool_id ? (int) $rule->pool_id : null);
            $state['pool_index']      = 0;
            $state['phase']           = 'search';
     ...
     * @return int[] ID пулов.
     */
    private function resolvePoolIds(int $companyId, ?int $accountId, ?array $limitAccountIds, ?int $rulePoolId = null): array
    {
        $db  = Yii::$app->db;
        ...
        if ($rulePoolId) {
            $sql .= " AND pool_id = " . (int) $rulePoolId;
        }
        if ($accountId) {
    ...
    private function poolPairsSelect(MatchingRule $rule, int $companyId, string $accList, string $filterA): string
    {
        ...
        $baseConditions    = $this->buildBaseConditions($rule);
        ...
        $idPairExprs = $this->buildIdPairExprs($rule);
    ...
    public function runRule(MatchingRule $rule, int $companyId, ?int $accountId, ?array $limitAccountIds = null): int
    {
        ...
        if ($rule->pool_id) {
            $poolQuery .= " AND pool_id = " . (int) $rule->pool_id;
        }
        if ($accountId) {
...
GROUP BY ls;";
    ...
    private function buildIdPairExprs(MatchingRule $rule): array
    {
        ...
        if ($rule->cross_id_search) {
            // Перекрёстный: каждое ID-поле A против каждого ID-поля B.
            foreach ($idFields as $fa) {
                foreach ($idFields as $fb) {
                    $exprs[] = "a.{$fa} IS NOT NULL AND a.{$fa} = b.{$fb}";
                }
            }
        } else {
            // Прямой: одноимённые поля.
            foreach ($idFields as $f) {
                $exprs[] = "a.{$f} IS NOT NULL AND a.{$f} = b.{$f}";
            }
        }
        return $exprs;
    }
